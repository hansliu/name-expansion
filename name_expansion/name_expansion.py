...
class NameExpansion(object):
  '''
  The NameExpansion is tool to expand string between '[]' and '{}'
  ...
  def parse(self, text):
    ...
    >>> NameExpansion().parse('tw[1,2].host[1-3,5].com')
    ['[1,2]', '[1-3,5]']

    >>> NameExpansion().parse('tw{1,2}.host{1-3,5}.com')
    ['{1,2}', '{1-3,5}']

    >>> NameExpansion().parse('[tw,hk].host.[com]')
    ['[tw,hk]', '[com]']
  ...
  def expand(self, match, data_list):
    ...
    >>> NameExpansion().expand('[1-3,5]', ['tw.host[1-3,5].com'])
    ['tw.host1.com', 'tw.host2.com', 'tw.host3.com', 'tw.host5.com']

    >>> NameExpansion().expand('[1-3|5]', ['tw.host[1-3|5].com'])
    ['tw.host1.com', 'tw.host2.com', 'tw.host3.com', 'tw.host5.com']

    >>> NameExpansion().expand('[tw,hk]', ['[tw,hk].host.com'])
    ...
    for data in data_list:
      ...
      for item in re.split('[,\|]', match[1:len(match)-1]):
        if '-' in item:
          seq = item.split('-')
